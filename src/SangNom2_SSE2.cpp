...
static AVS_FORCEINLINE __m128i _mm_packus_epi32_sse2(const __m128i& v1, const __m128i& v2) noexcept
{
...
static AVS_FORCEINLINE __m128i convSign16(const __m128i& v1) noexcept
{
...
static AVS_FORCEINLINE __m128i sse_load_si128(const T* ptr) noexcept
{
    if (aligned)
...
template <typename T, bool aligned>
static AVS_FORCEINLINE __m128 sse_load_ps(const T* ptr) noexcept
{
    if (aligned)
...
template <typename T, bool aligned>
static AVS_FORCEINLINE void sse_store_si128(T* ptr, __m128i val) noexcept
{
    if (aligned)
...
template <typename T, bool aligned>
static AVS_FORCEINLINE void sse_store_ps(T* ptr, __m128 val) noexcept
{
    if (aligned)
...
static AVS_FORCEINLINE __m128i sse_load_1_to_left_si128(const T* ptr) noexcept
{
...
template <typename T, bool isBorder, bool alignedLoad>
static AVS_FORCEINLINE __m128 sse_load_1_to_left_ps(const T* ptr) noexcept
{
...
template <typename T, bool isBorder, bool alignedLoad>
static AVS_FORCEINLINE __m128i sse_load_2_to_left_si128(const T* ptr) noexcept
{
    static_assert(std::is_integral<T>::value && (sizeof(T) == 1 || sizeof(T) == 2 || sizeof(T) == 4),
        "error instantiation, T must be 8, 16, 32 bit int");
...
template <typename T, bool isBorder, bool alignedLoad>
static AVS_FORCEINLINE __m128 sse_load_2_to_left_ps(const T* ptr) noexcept
{
    static_assert(std::is_floating_point<T>::value && (sizeof(T) == 4),
        "error instantiation, T must be 32 bit float");
...
template <typename T, bool isBorder, bool alignedLoad>
static AVS_FORCEINLINE __m128i sse_load_3_to_left_si128(const T* ptr) noexcept
{
    static_assert(std::is_integral<T>::value && (sizeof(T) == 1 || sizeof(T) == 2 || sizeof(T) == 4),
        "error instantiation, T must be 8, 16, 32 bit int");
...
template <typename T, bool isBorder, bool alignedLoad>
static AVS_FORCEINLINE __m128 sse_load_3_to_left_ps(const T* ptr) noexcept
{
    static_assert(std::is_floating_point<T>::value && (sizeof(T) == 4),
        "error instantiation, T must be 32 bit float");
...
template <typename T, bool isBorder, bool alignedLoad>
static AVS_FORCEINLINE __m128i sse_load_1_to_right_si128(const T* ptr) noexcept
{
    static_assert(std::is_integral<T>::value && (sizeof(T) == 1 || sizeof(T) == 2 || sizeof(T) == 4),
        "error instantiation, T must be 8, 16, 32 bit int");
...
template <typename T, bool isBorder, bool alignedLoad>
static AVS_FORCEINLINE __m128 sse_load_1_to_right_ps(const T* ptr) noexcept
{
    static_assert(std::is_floating_point<T>::value && (sizeof(T) == 4),
        "error instantiation, T must be 32 bit float");
...
template <typename T, bool isBorder, bool alignedLoad>
static AVS_FORCEINLINE __m128i sse_load_2_to_right_si128(const T* ptr) noexcept
{
    static_assert(std::is_integral<T>::value && (sizeof(T) == 1 || sizeof(T) == 2 || sizeof(T) == 4),
        "error instantiation, T must be 8, 16, 32 bit int");
...
template <typename T, bool isBorder, bool alignedLoad>
static AVS_FORCEINLINE __m128 sse_load_2_to_right_ps(const T* ptr) noexcept
{
    static_assert(std::is_floating_point<T>::value && (sizeof(T) == 4),
        "error instantiation, T must be 32 bit float");
...
template <typename T, bool isBorder, bool alignedLoad>
static AVS_FORCEINLINE __m128i sse_load_3_to_right_si128(const T* ptr) noexcept
{
    static_assert(std::is_integral<T>::value && (sizeof(T) == 1 || sizeof(T) == 2 || sizeof(T) == 4),
        "error instantiation, T must be 8, 16, 32 bit int");
...
template <typename T, bool isBorder, bool alignedLoad>
static AVS_FORCEINLINE __m128 sse_load_3_to_right_ps(const T* ptr) noexcept
{
    static_assert(std::is_floating_point<T>::value && (sizeof(T) == 4),
        "error instantiation, T must be 32 bit float");
...
static AVS_FORCEINLINE Arg sse_abs_diff(Arg a, Arg b) noexcept;

template <>
AVS_FORCEINLINE __m128i sse_abs_diff<uint8_t, __m128i>(__m128i a, __m128i b) noexcept
{
...
template <>
AVS_FORCEINLINE __m128i sse_abs_diff<uint16_t, __m128i>(__m128i a, __m128i b) noexcept
{
...
template <>
AVS_FORCEINLINE __m128 sse_abs_diff<float, __m128>(__m128 a, __m128 b) noexcept
{
...
template <typename T, typename Arg>
AVS_FORCEINLINE Arg calculateSangNom_sse(const Arg& p1, const Arg& p2, const Arg& p3) noexcept;

template <>
AVS_FORCEINLINE __m128i calculateSangNom_sse<uint8_t, __m128i>(const __m128i& p1, const __m128i& p2, const __m128i& p3) noexcept
{
...
template <>
AVS_FORCEINLINE __m128i calculateSangNom_sse<uint16_t, __m128i>(const __m128i& p1, const __m128i& p2, const __m128i& p3) noexcept
{
    const auto const_0{ _mm_setzero_si128() };
...
template <>
AVS_FORCEINLINE __m128 calculateSangNom_sse<float, __m128>(const __m128& p1, const __m128& p2, const __m128& p3) noexcept
{
...
template <typename T, typename Arg>
static AVS_FORCEINLINE Arg getAvgIfMinBuf(const Arg& a1, const Arg& a2, const Arg& buf, const Arg& minBuf, const Arg& acc) noexcept;

template <>
AVS_FORCEINLINE __m128i getAvgIfMinBuf<uint8_t, __m128i>(const __m128i& a1, const __m128i& a2, const __m128i& buf, const __m128i& minBuf, const __m128i& acc) noexcept
{
...
template <>
AVS_FORCEINLINE __m128i getAvgIfMinBuf<uint16_t, __m128i>(const __m128i& a1, const __m128i& a2, const __m128i& buf, const __m128i& minBuf, const __m128i& acc) noexcept
{
...
template <>
AVS_FORCEINLINE __m128 getAvgIfMinBuf<float, __m128>(const __m128& a1, const __m128& a2, const __m128& buf, const __m128& minBuf, const __m128& acc) noexcept
{
...
static AVS_FORCEINLINE void prepareBuffersLine_sse(const uint8_t* srcp, const uint8_t* srcpn2, uint8_t* buffers[TOTAL_BUFFERS], const int w, const int bufferOffset) noexcept
{
    ...
    for (int x{ 0 }; x < w; x += pixelStep)
    {
        ...
        sse_store_si128<uint8_t, alignedStore>((buffers[as_int(Buffers::ADIFF_M3_P3)] + bufferOffset + x), sse_abs_diff<uint8_t, __m128i>(currLineM3, nextLineP3));
        sse_store_si128<uint8_t, alignedStore>((buffers[as_int(Buffers::ADIFF_M2_P2)] + bufferOffset + x), sse_abs_diff<uint8_t, __m128i>(currLineM2, nextLineP2));
        sse_store_si128<uint8_t, alignedStore>((buffers[as_int(Buffers::ADIFF_M1_P1)] + bufferOffset + x), sse_abs_diff<uint8_t, __m128i>(currLineM1, nextLineP1));
        sse_store_si128<uint8_t, alignedStore>((buffers[as_int(Buffers::ADIFF_P0_M0)] + bufferOffset + x), sse_abs_diff<uint8_t, __m128i>(currLine, nextLine));
        sse_store_si128<uint8_t, alignedStore>((buffers[as_int(Buffers::ADIFF_P1_M1)] + bufferOffset + x), sse_abs_diff<uint8_t, __m128i>(currLineP1, nextLineM1));
        sse_store_si128<uint8_t, alignedStore>((buffers[as_int(Buffers::ADIFF_P2_M2)] + bufferOffset + x), sse_abs_diff<uint8_t, __m128i>(currLineP2, nextLineM2));
        sse_store_si128<uint8_t, alignedStore>((buffers[as_int(Buffers::ADIFF_P3_M3)] + bufferOffset + x), sse_abs_diff<uint8_t, __m128i>(currLineP3, nextLineM3));

        sse_store_si128<uint8_t, alignedStore>((buffers[as_int(Buffers::SG_FORWARD)] + bufferOffset + x), sse_abs_diff<uint8_t, __m128i>(forwardSangNom1, forwardSangNom2));
        sse_store_si128<uint8_t, alignedStore>((buffers[as_int(Buffers::SG_REVERSE)] + bufferOffset + x), sse_abs_diff<uint8_t, __m128i>(backwardSangNom1, backwardSangNom2));
    }
}

template <BorderMode border, bool alignedLoad, bool alignedStore>
static AVS_FORCEINLINE void prepareBuffersLine_sse(const uint16_t* srcp, const uint16_t* srcpn2, uint16_t* buffers[TOTAL_BUFFERS], const int w, const int bufferOffset) noexcept
{
    ...
    for (int x{ 0 }; x < w; x += pixelStep)
    {
        ...
        sse_store_si128<uint16_t, alignedStore>((buffers[as_int(Buffers::ADIFF_M3_P3)] + bufferOffset + x), sse_abs_diff<uint16_t, __m128i>(currLineM3, nextLineP3));
        sse_store_si128<uint16_t, alignedStore>((buffers[as_int(Buffers::ADIFF_M2_P2)] + bufferOffset + x), sse_abs_diff<uint16_t, __m128i>(currLineM2, nextLineP2));
        sse_store_si128<uint16_t, alignedStore>((buffers[as_int(Buffers::ADIFF_M1_P1)] + bufferOffset + x), sse_abs_diff<uint16_t, __m128i>(currLineM1, nextLineP1));
        sse_store_si128<uint16_t, alignedStore>((buffers[as_int(Buffers::ADIFF_P0_M0)] + bufferOffset + x), sse_abs_diff<uint16_t, __m128i>(currLine, nextLine));
        sse_store_si128<uint16_t, alignedStore>((buffers[as_int(Buffers::ADIFF_P1_M1)] + bufferOffset + x), sse_abs_diff<uint16_t, __m128i>(currLineP1, nextLineM1));
        sse_store_si128<uint16_t, alignedStore>((buffers[as_int(Buffers::ADIFF_P2_M2)] + bufferOffset + x), sse_abs_diff<uint16_t, __m128i>(currLineP2, nextLineM2));
        sse_store_si128<uint16_t, alignedStore>((buffers[as_int(Buffers::ADIFF_P3_M3)] + bufferOffset + x), sse_abs_diff<uint16_t, __m128i>(currLineP3, nextLineM3));

        sse_store_si128<uint16_t, alignedStore>((buffers[as_int(Buffers::SG_FORWARD)] + bufferOffset + x), sse_abs_diff<uint16_t, __m128i>(forwardSangNom1, forwardSangNom2));
        sse_store_si128<uint16_t, alignedStore>((buffers[as_int(Buffers::SG_REVERSE)] + bufferOffset + x), sse_abs_diff<uint16_t, __m128i>(backwardSangNom1, backwardSangNom2));
    }
}

template <BorderMode border, bool alignedLoad, bool alignedStore>
static AVS_FORCEINLINE void prepareBuffersLine_sse(const float* srcp, const float* srcpn2, float* buffers[TOTAL_BUFFERS], const int w, const int bufferOffset) noexcept
{
    ...
    for (int x{ 0 }; x < w; x += pixelStep)
    {
        ...
        sse_store_ps<float, alignedStore>((buffers[as_int(Buffers::ADIFF_M3_P3)] + bufferOffset + x), sse_abs_diff<float, __m128>(currLineM3, nextLineP3));
        sse_store_ps<float, alignedStore>((buffers[as_int(Buffers::ADIFF_M2_P2)] + bufferOffset + x), sse_abs_diff<float, __m128>(currLineM2, nextLineP2));
        sse_store_ps<float, alignedStore>((buffers[as_int(Buffers::ADIFF_M1_P1)] + bufferOffset + x), sse_abs_diff<float, __m128>(currLineM1, nextLineP1));
        sse_store_ps<float, alignedStore>((buffers[as_int(Buffers::ADIFF_P0_M0)] + bufferOffset + x), sse_abs_diff<float, __m128>(currLine, nextLine));
        sse_store_ps<float, alignedStore>((buffers[as_int(Buffers::ADIFF_P1_M1)] + bufferOffset + x), sse_abs_diff<float, __m128>(currLineP1, nextLineM1));
        sse_store_ps<float, alignedStore>((buffers[as_int(Buffers::ADIFF_P2_M2)] + bufferOffset + x), sse_abs_diff<float, __m128>(currLineP2, nextLineM2));
        sse_store_ps<float, alignedStore>((buffers[as_int(Buffers::ADIFF_P3_M3)] + bufferOffset + x), sse_abs_diff<float, __m128>(currLineP3, nextLineM3));

        sse_store_ps<float, alignedStore>((buffers[as_int(Buffers::SG_FORWARD)] + bufferOffset + x), sse_abs_diff<float, __m128>(forwardSangNom1, forwardSangNom2));
        sse_store_ps<float, alignedStore>((buffers[as_int(Buffers::SG_REVERSE)] + bufferOffset + x), sse_abs_diff<float, __m128>(backwardSangNom1, backwardSangNom2));
    }
}

template <typename T, typename IType>
static AVS_FORCEINLINE void prepareBuffers_sse(const T* srcp, const int srcStride, const int w, const int h, const int bufferStride, T* buffers[TOTAL_BUFFERS]) noexcept
{
...
static AVS_FORCEINLINE void processBuffersBlock_sse(uint8_t* bufferp, const int16_t* bufferLine, const int x) noexcept
{
...
template <BorderMode border>
static AVS_FORCEINLINE void processBuffersBlock_sse(uint16_t* bufferp, const int32_t* bufferLine, const int x) noexcept
{
...
template <BorderMode border>
static AVS_FORCEINLINE void processBuffersBlock_sse(float* bufferp, const float* bufferLine, const int x) noexcept
{
...
static AVS_FORCEINLINE void processBuffers_sse(uint8_t* bufferp, int16_t* bufferLine, const int bufferStride, const int bufferHeight) noexcept
{
...
static AVS_FORCEINLINE void processBuffers_sse(uint16_t* bufferp, int32_t* bufferLine, const int bufferStride, const int bufferHeight) noexcept
{
    auto bufferpp1{ bufferp };
    auto bufferpc0{ bufferpp1 + bufferStride };
    auto bufferpn1{ bufferpc0 + bufferStride };
...
static AVS_FORCEINLINE void processBuffers_sse(float* bufferp, float* bufferLine, const int bufferStride, const int bufferHeight) noexcept
{
    auto bufferpp1{ bufferp };
    auto bufferpc0{ bufferpp1 + bufferStride };
    auto bufferpn1{ bufferpc0 + bufferStride };
...
static AVS_FORCEINLINE void finalizePlaneLine_sse(const uint8_t* srcp, const uint8_t* srcpn2, uint8_t* dstpn, uint8_t* buffers[TOTAL_BUFFERS], int bufferOffset, const int w, const int pixelStep, const float aaf) noexcept
{
    ...
    for (int x{ 0 }; x < w; x += pixelStep)
    {
        ...
        auto buf0{ sse_load_si128<uint8_t, alignedLoadBuffer>(buffers[as_int(Buffers::ADIFF_M3_P3)] + bufferOffset + x) };
        auto buf1{ sse_load_si128<uint8_t, alignedLoadBuffer>(buffers[as_int(Buffers::ADIFF_M2_P2)] + bufferOffset + x) };
        auto buf2{ sse_load_si128<uint8_t, alignedLoadBuffer>(buffers[as_int(Buffers::ADIFF_M1_P1)] + bufferOffset + x) };
        auto buf3{ sse_load_si128<uint8_t, alignedLoadBuffer>(buffers[as_int(Buffers::SG_FORWARD)] + bufferOffset + x) };
        auto buf4{ sse_load_si128<uint8_t, alignedLoadBuffer>(buffers[as_int(Buffers::ADIFF_P0_M0)] + bufferOffset + x) };
        auto buf5{ sse_load_si128<uint8_t, alignedLoadBuffer>(buffers[as_int(Buffers::SG_REVERSE)] + bufferOffset + x) };
        auto buf6{ sse_load_si128<uint8_t, alignedLoadBuffer>(buffers[as_int(Buffers::ADIFF_P1_M1)] + bufferOffset + x) };
        auto buf7{ sse_load_si128<uint8_t, alignedLoadBuffer>(buffers[as_int(Buffers::ADIFF_P2_M2)] + bufferOffset + x) };
        auto buf8{ sse_load_si128<uint8_t, alignedLoadBuffer>(buffers[as_int(Buffers::ADIFF_P3_M3)] + bufferOffset + x) };
...
template <BorderMode border, bool alignedLoad, bool alignedLoadBuffer, bool alignedStore>
static AVS_FORCEINLINE void finalizePlaneLine_sse(const uint16_t* srcp, const uint16_t* srcpn2, uint16_t* dstpn, uint16_t* buffers[TOTAL_BUFFERS], int bufferOffset, const int w, const int pixelStep, const float aaf) noexcept
{
    ...
    for (int x{ 0 }; x < w; x += pixelStep)
    {
        ...
        auto buf0{ sse_load_si128<uint16_t, alignedLoadBuffer>(buffers[as_int(Buffers::ADIFF_M3_P3)] + bufferOffset + x) };
        auto buf1{ sse_load_si128<uint16_t, alignedLoadBuffer>(buffers[as_int(Buffers::ADIFF_M2_P2)] + bufferOffset + x) };
        auto buf2{ sse_load_si128<uint16_t, alignedLoadBuffer>(buffers[as_int(Buffers::ADIFF_M1_P1)] + bufferOffset + x) };
        auto buf3{ sse_load_si128<uint16_t, alignedLoadBuffer>(buffers[as_int(Buffers::SG_FORWARD)] + bufferOffset + x) };
        auto buf4{ sse_load_si128<uint16_t, alignedLoadBuffer>(buffers[as_int(Buffers::ADIFF_P0_M0)] + bufferOffset + x) };
        auto buf5{ sse_load_si128<uint16_t, alignedLoadBuffer>(buffers[as_int(Buffers::SG_REVERSE)] + bufferOffset + x) };
        auto buf6{ sse_load_si128<uint16_t, alignedLoadBuffer>(buffers[as_int(Buffers::ADIFF_P1_M1)] + bufferOffset + x) };
        auto buf7{ sse_load_si128<uint16_t, alignedLoadBuffer>(buffers[as_int(Buffers::ADIFF_P2_M2)] + bufferOffset + x) };
        auto buf8{ sse_load_si128<uint16_t, alignedLoadBuffer>(buffers[as_int(Buffers::ADIFF_P3_M3)] + bufferOffset + x) };
...
template <BorderMode border, bool alignedLoad, bool alignedLoadBuffer, bool alignedStore>
static AVS_FORCEINLINE void finalizePlaneLine_sse(const float* srcp, const float* srcpn2, float* dstpn, float* buffers[TOTAL_BUFFERS], int bufferOffset, const int w, const int pixelStep, const float aaf) noexcept
{
    ...
    for (int x{ 0 }; x < w; x += pixelStep)
    {
        ...
        auto buf0{ sse_load_ps<float, alignedLoadBuffer>(buffers[as_int(Buffers::ADIFF_M3_P3)] + bufferOffset + x) };
        auto buf1{ sse_load_ps<float, alignedLoadBuffer>(buffers[as_int(Buffers::ADIFF_M2_P2)] + bufferOffset + x) };
        auto buf2{ sse_load_ps<float, alignedLoadBuffer>(buffers[as_int(Buffers::ADIFF_M1_P1)] + bufferOffset + x) };
        auto buf3{ sse_load_ps<float, alignedLoadBuffer>(buffers[as_int(Buffers::SG_FORWARD)] + bufferOffset + x) };
        auto buf4{ sse_load_ps<float, alignedLoadBuffer>(buffers[as_int(Buffers::ADIFF_P0_M0)] + bufferOffset + x) };
        auto buf5{ sse_load_ps<float, alignedLoadBuffer>(buffers[as_int(Buffers::SG_REVERSE)] + bufferOffset + x) };
        auto buf6{ sse_load_ps<float, alignedLoadBuffer>(buffers[as_int(Buffers::ADIFF_P1_M1)] + bufferOffset + x) };
        auto buf7{ sse_load_ps<float, alignedLoadBuffer>(buffers[as_int(Buffers::ADIFF_P2_M2)] + bufferOffset + x) };
        auto buf8{ sse_load_ps<float, alignedLoadBuffer>(buffers[as_int(Buffers::ADIFF_P3_M3)] + bufferOffset + x) };
...
template <typename T, typename IType>
static AVS_FORCEINLINE void finalizePlane_sse(T* dstp, const int dstStride, const int w, const int h, const int bufferStride, const T aaf, T* buffers[TOTAL_BUFFERS]) noexcept
{
    auto srcp{ dstp };
...
template <typename T, typename IType>
void SangNom2::sangnom_sse(uint8_t* __restrict dstp_, const int dstStride, const int w, const int h, int offset, int plane) noexcept
{
    ...
    prepareBuffers_sse<T, IType>(dstp + static_cast<int64_t>(offset) * dstStride, dstStride, w, h, buffer_stride, buffers);

    for (int i{ 0 }; i < TOTAL_BUFFERS; ++i)
        processBuffers_sse(buffers[i], bufferLine, buffer_stride, bufferHeight);

    finalizePlane_sse<T, IType>(dstp + static_cast<int64_t>(offset) * dstStride, dstStride, w, h, buffer_stride, aaf[plane], buffers);
}

template void SangNom2::sangnom_sse<uint8_t, int16_t>(uint8_t* __restrict dstp_, const int dstStride, const int w, const int h, int offset, int plane) noexcept;
template void SangNom2::sangnom_sse<uint16_t, int32_t>(uint8_t* __restrict dstp_, const int dstStride, const int w, const int h, int offset, int plane) noexcept;
template void SangNom2::sangnom_sse<float, float>(uint8_t* __restrict dstp_, const int dstStride, const int w, const int h, int offset, int plane) noexcept;
